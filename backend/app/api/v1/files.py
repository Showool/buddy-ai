...
async def upload_file(
    ...
    """
    logger.info(
        f"收到文件上传请求: user_id={user_id}, filename={file.filename}, is_public={is_public}"
    )
    ...
    try:
        ...
        asyncio.create_task(
            vectorization_service.vectorize_file(
                saved_file_id, user_id, file.filename or "", is_public
            )
        )

        logger.info(
            f"文件上传成功: {file.filename} (用户: {user_id}, ID: {saved_file_id}, 任务ID: {task_id})"
        )
...
async def list_files(user_id: str = Query(..., description="用户ID，用于过滤文件")):
    """
    列出已上传的文件
...
async def delete_file(
    file_id: str, user_id: str = Query(..., description="用户ID，用于权限验证")
):
    """
    删除文件
    ...
    if user_file.user_id != user_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="没有权限删除此文件"
        )
    ...
    if deleted:
        return DeleteFileResponse(
            status="success", message="文件及向量数据已删除", file_id=file_id
        )
    else:
        raise HTTPException(
...
async def get_file_info(
    ...
    if user_file.user_id != user_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="没有权限访问此文件"
        )
...
async def get_vectorization_progress(file_id: str):
    ...
    return {
        ...
        "processed_chunks": (
            user_file.chunk_count or 0 if vec_status == "completed" else 0
        ),
        "error_message": None,
    }
        ...
        "private": [
            {
                ...
                "summary": (
                    f.document_summary if hasattr(f, "document_summary") else None
                ),
                "last_vectorized": (
                    f.last_vectorized_at if hasattr(f, "last_vectorized_at") else None
                ),

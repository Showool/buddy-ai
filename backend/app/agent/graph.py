...
def get_graph():
    ...
    with (RedisSaver.from_conn_string(settings.REDIS_URL) as checkpointer,
        ...
        workflow.add_node("retrieval_documents", document_retrieval_node)
        ...
        workflow.add_conditional_edges(
            "memory_retrieval",
            lambda state: "retrieval_documents" if state.get("retrieval_needed", {}).get("rag", False) else "generate_response",
            {
        ...
        workflow.add_edge("retrieval_documents", "grade_documents")
        ...
        workflow.add_edge("rewrite_question", "retrieval_documents")

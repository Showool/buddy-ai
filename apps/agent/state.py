...
class RouteSchema(BaseModel):
    route_decision: Literal["answer_directly", "knowledge_base_search", "plan_and_execute"] = Field(
        ..., description="Classification of user input information"
    )
...
def merge_rag_docs(existing: list[dict[str, Any]] | None, new: list[dict[str, Any]] | None) -> list[dict[str, Any]]:
    if existing is None:
        return new or []
    if new is None:
        return existing
...
class GraphState(MessagesState):
    ...
    rag_docs: Annotated[list[dict[str, Any]], merge_rag_docs]
